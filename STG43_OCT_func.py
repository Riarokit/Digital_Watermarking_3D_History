...
# --------------------------ここから点の間に電子透かし埋め込む関数---------------------------------
def select_embeddable_random(binary_string_check,input_path_location,input_path_color=None,max_size=1):
    """
    ...
    min_size,voxel_index = create_index(bit_stream,level_ptrs,1,max_depth,level_bits_list,
                                        reconstruct_count,points,voxel_info,max_size,voxel_index)
    ...
    return pcd,embed_points

def select_embeddable_between(input_path_location,input_path_color=None,max_size=1):
    """
...
def create_index(bit_stream,level_ptrs,current_depth,max_depth,level_bits_list,
                 reconstruct_count,points,voxel_info,size,voxel_index,origin=np.array([0,0,0]),min_size=None):
    """
        ...
        if bit == '1':
            ...
            # 現在最深階層以外なら再帰処理で階層を進む
                ...
                min_size,voxel_index = create_index(bit_stream, level_ptrs,current_depth + 1,max_depth,level_bits_list,
                                                    reconstruct_count,points,voxel_info,size/2.0,voxel_index,next_origin,min_size)
        else:
            if current_depth == max_depth:
                voxel_offset = np.array([i & 1, (i >> 1) & 1, (i >> 2) & 1],dtype=np.float32) * (size/2.0)
                point = origin + voxel_offset
...
def find_choose_embedding_random(voxel_info, binary_string_check):
    ...
    # coordinate をリストに保存
    embed_points = [np.array(item['coordinate']) for item in selected_items]
    return embed_points
...
def find_embedding_between(voxel_info, voxelsize):
    ...
    embedding_candidate ([{"target", "hit", "coord"}]): 埋め込み候補位置の情報の辞書型リスト
    """
    # voxelsize * search_range_coefficient以内の範囲でhitを探索
    search_range_coefficient = 3
        ...
        if target['exist'] == 1:
            ...
            for hit in voxel_info:
                if hit['exist'] == 1 and hit != target:
                    ...
                    # 軸ごとの距離チェック
                    if any(
                        abs(target_coord[dim] - hit_coord[dim]) > voxelsize * search_range_coefficient
                        for dim in range(3)
                    ):
                        continue
                    # ユークリッド距離チェック
                    distance = np.linalg.norm(np.array(target_coord) - np.array(hit_coord))
                    if distance > voxelsize * search_range_coefficient:
                        continue
                    # 中点を計算
                    ...
                    for j, (min_coord, mid) in enumerate(zip(min_coords, midpoint)):
                        value = min_coord
                        while value + voxelsize < mid:
                            value += voxelsize
                        candidate_coord[j] = value
                    # 計算したcandidate_coordがiscloseで既存点と同じと判定されなければ埋め込み候補点として追加
                    if not any(np.all(np.isclose(candidate_coord, existing)) for existing in existing_points):
...
def extract_bits_from_candidates(pcd, embed_points):
    ...
    print(f"最終的な追加点の数: {count_of_ones}")
