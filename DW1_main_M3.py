...
if __name__ == "__main__":
    """
    main関数概要:
    点群をクラスタリングし、各クラスタの点の座標ベクトルを法線方向に射影したときの大きさを信号値としてGFTを行う。
    ...
    normals = DW1F.estimate_normals_xyz(xyz, knn=30, orient_knn=30, make_outward=True)
    ...
    # 5. 単多数決方式の埋め込み
    xyz_after = DW1F.embed_watermark_normal(
        xyz, normals, labels, watermark_bits, beta=beta,
    ...
    # 6. 単多数決方式の抽出
    start = time.time()
    extracted_bits = DW1F.extract_watermark_normal(
        xyz_after, xyz, normals, labels, watermark_bits_length,
        graph_mode=graph_mode, k=k, radius=radius,
        min_spectre=min_spectre, max_spectre=max_spectre
    )
