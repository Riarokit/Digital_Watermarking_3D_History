...
import STG11_DCT_func as STG11F
import time
...
# ボクセル化後の点群を可視化
STG11F.vis_cust(before_voxels, window_name="ボクセル化後の点群")
...
# NxNxN配列を作成
voxel_dct = STG11F.make_all_voxels(voxels)
...
STG11F.vis_cust(voxel_dct_vis, window_name="N*N*Nの立方体")

# DCT変換
print("-------------dct開始--------------")
start_dct = time.time()
dctcoef = STG11F.dct_3d(voxel_dct)
print("-------------dct完了--------------")
...
STG11F.vis_cust(dct_vis, window_name="DCT変換後の立方体")
...
print("-------------埋め込み開始------------")
start_emb = time.time()
message = "HelloWorld, MorinoLab2024"

# 文字列をASCIIコードの2進数に変換
binary_message = STG11F.string_to_binary(message)

seed = 1  # 固定シードを使用して一貫性を保つ
psnr = 40  # PSNR値を設定
percentage = 2  # 上位x%からランダムに選択

dctcoef_emb, positions, original_dctcoef, a = STG11F.embed_string(dctcoef, message, percentage, psnr, seed)
print("------------埋め込み完了-------------")
...
STG11F.vis_cust(emb_vis, window_name="埋め込み挿入後の立方体")
...
# 圧縮
com = STG11F.comp(dctcoef_emb, 0.6)

# 逆DCT変換
print("--------------idct開始---------------")
start_idct = time.time()
voxel_dct2 = STG11F.idct_3d(com)
print("--------------idct完了---------------")
...
STG11F.vis_cust(idct_vis, window_name="逆DCT変換後の立方体")

# 透かし視覚化
after_voxels = STG11F.visualize(voxel_dct2, voxel_num, voxel_size)
STG11F.vis_cust(after_voxels, window_name="透かし入りのボクセル")

# 逆DCT変換後の点群を再度ボクセル化せずにそのままDCT変換
print("---------------再度DCT変換開始---------------")
start_dct2 = time.time()
dctcoef_after_idct = STG11F.dct_3d(voxel_dct2)
print("---------------再度DCT変換完了----------------")
...
detected_binary_message = STG11F.detect_string(dctcoef_after_idct, positions, original_dctcoef, a)
...
if all(bit in '01' for bit in detected_binary_message):
    try:
        ascii_message = STG11F.binary_to_string(detected_binary_message)
